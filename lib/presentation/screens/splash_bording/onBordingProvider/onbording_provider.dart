...
class OnBordingProvider with ChangeNotifier {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    RichText(
                      ...
                      TextSpan(
                          text: AppLocalizations.of(context)?.translate("We have sent the OTP to") ??
                              "We have sent the OTP to",
                          style: Theme.of(context).textTheme.bodyMedium),
                      TextSpan(
                          text: " +$ccode ${mobileNumber.text}", style: Theme.of(context).textTheme.bodyMedium),
                    ])),
                    ...
                    OTPTextField(
                      ...
                      otpFieldStyle: OtpFieldStyle(
                          ...
                          errorBorderColor: Theme.of(context).dividerTheme.color!,
                          focusBorderColor: AppColors.appColor,
                          enabledBorderColor: Theme.of(context).dividerTheme.color!,
                          disabledBorderColor: Theme.of(context).dividerTheme.color!,
                      ...
                      onTap: () async {
                        try {
                          PhoneAuthCredential credential =
                              PhoneAuthProvider.credential(verificationId: vericitionId, smsCode: otp);
                          await FirebaseAuth.instance.signInWithCredential(credential).then((value) {
                            if (value.user != null) {
                              if (isForgot) {
                                ...
                                checkUserAfterOtpVerification(context, value.user!.phoneNumber ?? "");
                              }
                            }
                          });
                        } catch (e) {
                          ...
                          Fluttertoast.showToast(
                              msg: AppLocalizations.of(context)?.translate("OTP Invalid") ?? "OTP Invalid");
                        }
                      },
                    )
                  ],
  ...
  // New method to check user after OTP verification
  Future<void> checkUserAfterOtpVerification(context, String phoneNumber) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Fluttertoast.showToast(msg: response.data["ResponseMsg"] ?? "Something went wrong");
      }
    } catch (e) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  AppLocalizations.of(context)?.translate("Confirm Password") ?? "Confirm Password",
                  style: Theme.of(context).textTheme.bodySmall,
                ),
                const SizedBox(
                  height: 5,
                ...
                TextFieldPro(
                  controller: newPass,
                  hintText: AppLocalizations.of(context)?.translate("Confirm") ?? "Confirm",
                  textalingn: TextAlign.start,
                ),
                const SizedBox(
                  height: 20,
                    ...
                    Expanded(
                      ...
                      onTap: () {
                        if (pass.text == newPass.text) {
                          BlocProvider.of<OnbordingCubit>(context).forgotPassApi(
                              context: context, ccode: ccode, mobile: mobileNumber, password: newPass.text);
                        } else {
                          // Fluttertoast.showToast(msg: "Password Not Match".tr);
                          Fluttertoast.showToast(
                              msg: AppLocalizations.of(context)?.translate("Password Not Match") ??
                                  "Password Not Match");
                        }
                      },
                    )),
  ...
  searchLanguage(s) {
    ...
    for (int i = 0; i < languageModel!.languagelist!.length; i++) {
      if (languageModel!.languagelist![i].title.toString().toLowerCase().contains(s.toLowerCase())) {
  ...
  searchHobie(s) {
    ...
    for (int i = 0; i < getInterestModel!.interestlist!.length; i++) {
      if (getInterestModel!.interestlist![i].title.toString().toLowerCase().contains(s.toLowerCase())) {
