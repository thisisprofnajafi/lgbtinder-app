...
class LikeMatchProvider extends ChangeNotifier {
  ...
  Future profileViewApi({required String uid, required String profileId}) async {
    Map data = {"uid": uid, "profile_id": profileId};
    try {
      Response response = await _api.sendRequest.post("${Config.baseUrlApi}${Config.profileView}", data: data);
