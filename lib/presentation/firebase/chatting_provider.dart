...
class ChattingProvider extends ChangeNotifier {
    ...
    snep.forEach((element) {
      ...
      for (int a = 0; a < data.length; a++) {
        ...
        Stream<QuerySnapshot<Object?>> snapshot = chatservices.getMessage(
            userId: dataa["uid"], otherUserId: Provider.of<HomeProvider>(context, listen: false).uid);
  ...
  Widget buildMessageList({required String resiverUserId, required context}) {
    return StreamBuilder(
        stream: chatservices.getMessage(
            userId: resiverUserId, otherUserId: Provider.of<HomeProvider>(context, listen: false).uid),
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                AppLocalizations.of(context)?.translate("No messages here yet...") ??
                              ...
                              Text(
                                  AppLocalizations.of(context)
                                          ?.translate("Send a message or tap on the greeting below") ??
  ...
  Widget buildMessageiteam(DocumentSnapshot document, context) {
    ...
    var alingmentt = (data["senderid"] == Provider.of<HomeProvider>(context, listen: false).uid)
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: (data["senderid"] == Provider.of<HomeProvider>(context, listen: false).uid)
          ...
          children: [
            ...
            ChatBubble(
              ...
              alingment:
                  (data["senderid"] == Provider.of<HomeProvider>(context, listen: false).uid) ? false : true,
            ),
            ...
            Text(
                DateFormat('hh:mm a')
                    .format(DateTime.fromMicrosecondsSinceEpoch(data["timestamp"].microsecondsSinceEpoch))
                    .toString(),
        ...
        collectionReference.doc(resiverUserId).get().then((value) async {
          ...
          try {
            ...
            if (fields["isOnline"] == false) {
              sendPushMessage(
                  controller.text,
                  Provider.of<HomeProvider>(context, listen: false).userlocalData.userLogin!.name ?? "",
                  fmctoken,
                  context);
            } else {
              print("user online");
